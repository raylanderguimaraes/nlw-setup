...
export function HabitsList({ date, onCompletedChanged }: HabitListProps) {
    ...
    async function handleToggleHabit(habitId: string) {
        ...
        const isHabitAreadyCompleted = habitsInfo!.completedHabits.includes(habitId)
                ...
                habitsInfo?.possibleHabits.map(habit => {
                    return (
                        ...
                        >
                            ...
                            <span className="font-semibold text-xl text-white landing-tight group-data-[state=checked]:line-through group-data-[state=checked]:text-zinc-400">
                                {habit.title}
                            </span>
                        </Checkbox.Root>
                    )
